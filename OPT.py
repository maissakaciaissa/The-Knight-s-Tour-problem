...
def backtracking(knight):
    agenda = []  # Liste des arcs à explorer, inspirée de l'algorithme AC-3

    # Ajouter tous les mouvements possibles dans la liste agenda
    for direction in range(1, 9):
        if knight.consistent(direction):
            knight.add_move(direction)
            agenda.append(direction)

    while agenda:  # Exploration des directions dans l'agenda
        direction = agenda.pop()  # Prendre le dernier arc (mouvement) ajouté
        knight.add_move(direction)
        
        if len(knight.path) == 64:  # Si toutes les cases sont visitées
            return knight
        
        # Ajouter de nouveaux mouvements à explorer
        for direction in range(1, 9):
            if knight.consistent(direction):
                knight.add_move(direction)
                agenda.append(direction)
